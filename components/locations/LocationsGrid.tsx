...
// Generate image paths for a location
const getLocationImages = (slug: string, count: number): string[] => {
    return Array.from({ length: count }, (_, i) => `/Loaction images/${slug}/${i + 1}.JPG`);
};
            ...
            <div className="container mx-auto px-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {locations.map((loc, index) => (
                        <div
                            key={index}
                            className="group relative bg-card rounded-2xl text-center transition-all duration-700 hover:shadow-card gold-border-hover cursor-pointer flex flex-col items-center h-full overflow-hidden"
                            onClick={() => openBooking({ location: loc.name })}
                        >
                            {/* Image Slider */}
                            <LocationImageSlider
                                images={getLocationImages(loc.slug, loc.imageCount)}
                            />

                            {/* Card Content */}
                            <div className="p-8 pt-6 flex flex-col items-center w-full flex-grow">
                                {/* Icon Container */}
